...
## 2. Композиция
Помните, вопреки тому что рецепты изолированны и самодостаточны, их можно скомпоновать для последующего и постепенного превращения в более продвинутые выражения:
...
## 3. Побочные эффекты
"Побочный эффект" (Side effect) это основная концепция, которую следует рассмотреть перед тем как мы перечислим основные преобразования.
...
Это уже другая история — мы не можем изменить порядок выполнения, так в нашем терминале будет напечатано "barfoo" вместо "foobar" (и это явно не то, чего хотелось).
...
Хотя эти выражения и эквивалентны с позиции результирующего значения, они не эквивалентны в касательно побочных эффектов. Первое выражение добавит все элементы, а последнее отбросит все элементы как только найдет первое совпадающее с предикатом значение. Поэтому, такое упрощение сделать нельзя.
...
Поэтому, мы нужно либо избавиться от `builder`а (вместе с его API, в котором есть побочные эффекты), либо отделить вызов `builder`а от чистого выражения. Предположим, что этот `builder` является неким сторонним объектом, изжить который мы не можем, поэтому нам остается изолировать вызов:
...
Наименее очевидным, при этом наиболее важным преимуществом изоляции побочных эффектов будет улучшение надежности нашего кода, вне зависимости от других возможных оптимизаций. Касательно примера, первоначальное выражение может порождать различные побочные эффекты зависящие от текущей реализации `Seq`. Для `Vector`, например, оно добавит все элементы, для `Stream` оно пропустит все элементы после первого удачного сопоставления с предикатом (потому что стримы «ленивы» — элементы вычисляются только тогда, когда это необходимо). Отделение побочных эффектовпозволяет нам избежать этих неопределенностей.
...
Поскольку расчет размера коллекции может быть достаточно «дорогостоящим» вычислением для некоторых типов коллекций, мы можем сократить время сравнения с `O(length)` до `O(length min n)` для наследников `LinearSeq` (которые могут быть спрятаны под `Seq`-подобными значениями). Кроме того, тако2 подход незаменим когда имеем дело с бесконечными стримами.
...
У нас уже есть встроенный метод, который делает тоже самое. Оба выражения берут во внимание порядок элементов. И мы опять-таки сможем выиграть пользу от повышения производительности.
...
Второе выражение семантически эквивалентно, при этом более выразительно. Когда эти выражения приминяются к `Set`, производительность может разительно отличаться, из-за того что поиск у множеств стремится к `O(1)` (из-за внутреннего индексирования, не использующегося при вызове `exists`).
...
Вызов `filter` создает промежуточную коллекцию, которая занимает место в куче и нагружает GC. Более того, первые выражения находят все вхождения, в то время как требуется найти только первое (что может замедлить код, в зависимости от возможного содержимого коллекции). Потенциальный выигрыш в производительности менее значим для ленивых коллекций (таких как `Stream`, и в особенности `Iterator`).
...
Конечно, если `seq` не будет ленивой коллекцией (как, например, `Stream`), фильтрация найдет все вхождения (и создаст временную коллекцию), при том, что требовался только первый элемент.
Также применимо к: `Set`, `Option`, `Map`, `Iterator`.
...
При всей ясности и выразительности, последнее выражение может быть быстрее (потому что оно останаливает последующую обработку элементов после как только найдет первый подходящий элемент), что может работать для бесконечных последовательностей.
...
Это «классическая» в функциональном программировании реализация отображения (map) через светрку. Она поучительна, спору нет, также как и нет нужны ее использовать. Для этого у нас есть встроенный и выразительный метод (который еще и быстрее, так  как в своей реализации использует простой цикл `while`).
Также применимо к: `Set`, `Option`, `Iterator`.
...
Первое выражение создает промежуточную (перевернутую) коллекцию перед трансформацией элементов, что иногда бывает достаточно разумно (например для `List`). В других случаях, что будет более эффективно, можно сразу выполнить требуемые преобразования не создавая промежуточную коллекцию.
...
2) Трансформация порождающая коллекцию того же типа.
...
Также существует альтернативный способ совладать с «трансформацией + конверсией». И случай этот полагается на `breakOut`:
...
Имейте в виду, что упрощение это весьма [противоречиво](https://www.reddit.com/r/scala/comments/2z411u/scala_collections_tips_and_tricks/cqiip08/), в виду того что последнее выражение выглядит менее ясно, особенно если вы к нему не привыкли.
