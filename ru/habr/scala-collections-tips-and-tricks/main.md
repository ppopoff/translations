...
В этой статье представлен перечень упрощений и оптимизаций характерных для повседневного использования [API Scala коллекций](https://www.scala-lang.org/docu/files/collections-api/collections.html).

Некоторые советы основаны на тонкостях реализации библиотеки коллекций, однако, большинство рецептов являются разумными преобразованиями, которые на практике часто упускаются из виду.
...
Тем не менее, эти рецепты ценны сами по себе и могут помочь вам углубить ваше понимание стандартной библиотеки коллекций Scala и сделать ваш код быстрее и выразительнее.
...
## 2. Композиция
Имейте в виду, что несмотря на то, что все рецепты изолированны и самодостаточны, мы можем их скомпоновать с целью последующего итеративного превращения в более продвинутые выражения, например:
...
Таким образом мы можем полагаться на "заменяющую модель применения рецептов" (по аналогии с [SICP](https://mitpress.mit.edu/sicp/full-text/sicp/book/node10.html)), и использовать ее для упрощения сложных выражений.
