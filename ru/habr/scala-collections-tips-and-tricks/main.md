...
===
Because an instance of IndexedSeq[T] is also Function1[Int, T], you can use it directly as such.
===
...
#### Не используйте сортировку для находжения максимального элемента
...
### 4.9 Сопоставление
Вот несколько обособленных советов, посвященых [сопоставлению с образцом](http://docs.scala-lang.org/tutorials/tour/pattern-matching.html) в Scala и [частичным функциям](https://www.scala-lang.org/api/current/index.html#scala.PartialFunction).
...
Мы так жеможем применить обобщенный подход, полагающийся на использование представлений (смотрите ниже), получив: `seq.view.filter(p1).filter(p2).force`.
...
Мы так жеможем применить обобщенный подход который полагается на использование view (смотрите ниже), получив: `seq.view.map(f).map(g).force`.
...
Сортировка является затратной по вычислительности процедурой. Поэтому нет нужды сортировать элементы которые будут потенциально отфильтрованны на следующем шаге.
...
Первое выражение создает промежуточную (перевернутую) коллекцию передпреобразованием элементов, что иногда бывает достаточно разумным (например для `List`). В других случаях возможно непосредственно выполнить требуемые преобразования без создания промежуточной коллекции, что более эффективно.
...
Хотя основанный на видах подход и является более универсальным, в данномконкретном случае вы можете предпочесть метод `withFilter` из-за его лаконичности.
...
Это правило было представлено ранее (для последовтаельностей), однако для множеств существует более здравое обоснование.

Метод `sameElements` может возвращать неденерминированные результаты для неупорядоченных коллекций, потому что этот метод принимает во внимание порядок элементов, на который мы не можем полагаться в случае с множествами.
...
Because an instance of `Set[T]` is also `Function1[T, Boolean]`, you can use it directly as such.
===
